...
- **CDR**, **DCR**: not yet included — TBD whether these ship directly or via an external download link (see main repo README once added).
...
```matlab
run_group1          % Group 1: T=1, ADAM, all 12 (architecture, basis) configs, CDR
run_group1_CDR      % same, explicit CDR dataset argument
run_group1_DCR      % same, DCR dataset
run_all_table2_remaining   % Groups 2-6: remaining (dataset, T, optimizer) combinations
```
