function results = runExperiment_v2(dynamic,T,d,opti,basis,dataset,seed)
...
% seed    : OPTIONAL. Replaces the hardcoded rng(20) used for network
%           initialization and (for 'sgd') minibatch sampling. Omit this
%           argument entirely for the original, backward-compatible
%           behavior (rng(20), no seed suffix on the saved filename) --
%           this only exists to support multi-seed reruns (e.g. for
%           checking ADAM's run-to-run variance under
%           maxNumCompThreads(1)); it does not change GNvpro's behavior,
%           which is already deterministic and doesn't need reseeding.
%           Does NOT affect rng(42), which fixes the train/val/test data
%           split -- only the network-init/training seed varies.
%
...
% Pins MATLAB to single-threaded BLAS/LAPACK. Without this, dense matrix
% ops are multithreaded by default, and floating-point addition isn't
% associative -- thread-scheduling-dependent summation order made ADAM
% (10,000 iterations, lots of small matrix ops) genuinely non-reproducible
% run to run even with rng() fixed, while GNvpro (few, larger Newton
% steps) stayed deterministic. Confirmed fixed via
% isequal(r1.WOpt, r2.WOpt) on back-to-back identical-config runs.
maxNumCompThreads(1);
...
if not(exist('seed','var')) || isempty(seed)
    seed = [];   % sentinel: omitted -> use the original hardcoded seed below
end
...
if isempty(seed)
    rng(20);   % original hardcoded behavior, unchanged when seed is omitted
else
    rng(seed);
end
...
if isempty(seed)
    ...
    tag = sprintf('%s_%s_%s_d%d_T%g_%s_seed%d', config.dataset, dynamic, basis, d, T, opti, seed);
end
